...
public sealed class TicketApiClient : ITicketApiClient
{
    ...
    public async Task<HttpResponseMessage> DeleteAsync(int id, CancellationToken ct = default) =>
        ...
        public async Task<List<TicketDto>?> GetDeletedAsync(CancellationToken ct = default)
{
    return await _http.GetFromJsonAsync<List<TicketDto>>("api/tickets/deleted", ct);
}

public async Task<HttpResponseMessage> RestoreAsync(int id, CancellationToken ct = default)
{
    return await _http.PostAsync($"api/tickets/{id}/restore", null, ct);
}
}